...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        image_height, image_width, num_channels = state_shape[0], state_shape[1], state_shape[2]
        self.s = tf.placeholder(tf.uint8, shape=[None, image_height, image_width, num_channels*config.state_history],name='s')
        self.a = tf.placeholder(tf.int32, shape=[None],name='a')
        self.r = tf.placeholder(tf.float32, shape=[None],name='r')
        self.sp = tf.placeholder(tf.uint8, shape=[None, image_height, image_width, num_channels*config.state_history],name='sp')
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope):
            temp_state = tf.layers.flatten(state)
            out = tf.layers.dense(temp_state, num_actions, activation = None, use_bias = True,reuse=reuse, name=scope )
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        q_weights = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=q_scope)
        t_q_weights =  tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=target_q_scope)
        op = [tf.assign(t_q_weights[i], q_weights[i]) for i in range(len(q_weights))]
    ...
    def add_loss_op(self, q, target_q):
        ...
        HINT: 
            ...
            - You may find the following functions useful
                ...
                - tf.one_hot : The locations represented by indices in indices take value on_value, while all other locations take value off_value
    ...
    def add_optimizer_op(self, scope):
        ...
        gradients = optimizer.compute_gradients(self.loss, scope_variable)
        if self.config.grad_clip:
            clipped_gradients = [(tf.clip_by_norm(item[0],self.config.clip_val),item[1]) for item in gradients] 
            self.train_op = optimizer.apply_gradients(clipped_gradients)
            self.grad_norm = tf.global_norm([item[0] for item in clipped_gradients])
        else:
            self.train_op = optimizer.apply_gradients(gradients)
            self.grad_norm = tf.global_norm([item[0] for item in gradients])
        ##############################################################
        ######################## END YOUR CODE #######################
