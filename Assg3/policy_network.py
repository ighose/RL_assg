...
class PG(object):
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete == True:
      ...
      action_means = build_mlp(self.observation_placeholder,self.action_dim, scope, self.config.n_layers, self.config.layer_size, output_activation=None)
      std_dev = tf.get_variable('log_std', shape=(1, self.action_dim), dtype=tf.float32, initializer=tf.zeros_initializer(), trainable=True)
      log_std = tf.log(tf.exp(std_dev) + 1.0)
      log_std = tf.tile(log_std, [tf.shape(self.action_means)[0], 1])
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      s = 0
      l_rewards = len(rewards)
      returns = []
      for i in range(len(rewards)):
        if i == 0:
          returns.append(rewards[l_rewards-1-i])
        else:
          returns.append(rewards[l_rewards-1-i] + self.config.gamma * returns[i-1])
      returns.reverse()
      #######################################################
      #########          END YOUR CODE.          ############
