...
class BaselineNetwork(object):
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    baseline = build_mlp(self.observation_placeholder, 1, scope, self.config.n_layers, self.config.layer_size)
  ...
  def calculate_advantage(self, returns, observations):
    ...
    adv = returns
    adv = adv - self.sess.run(self.baseline, feed_dict={self.observation_placeholder : observations}).squeeze()
    #######################################################
    #########          END YOUR CODE.          ############
    return adv
